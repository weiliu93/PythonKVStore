...
def test_skiplist_basic_set_and_get():
    ...
    index = SkipListIndex(MemoryManager(pool_folder = pool_folder, conf_path = conf_path, block_file = block_file))
    index.set(1, 10)
    index.set(10, 100)
    index.set(7, 19)
...
def test_repeated_set_and_get_same_key():
    ...
    for _ in range(100):
        ...
        index.set(key, value)
        assert sorted([(key, value) for key, value in comparison_dict.items()]) == index.key_value_pairs()
...
def test_skiplist_real_scenario():
    ...
    for _ in range(20000):
        ...
        assert sorted(list(index.key_value_pairs())) == sorted([(key, value) for key, value in comparision_dict.items()])
...
def test_skiplist_clear():
    ...
    assert index.keys() == []
    assert len(index._heads) == 1 and not index._heads[0].right and not index._heads[0].down
...
def _get_test_case_package_path():
    ...
    return os.path.abspath(os.path.join(package_root_path, "skiplist_index", check_name))
...
def _get_common_file_paths():
    ...
    conf_path = os.path.abspath(
        os.path.join(package_root_path, "skiplist_index", check_name, "storage_conf.ini")
    )
