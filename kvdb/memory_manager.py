...
class MemoryManager(object):
    ...
    def __init__(self, conf_path = None, pool_folder=None, block_file=None):
        ...
        conf_path = conf_path or os.path.join(os.path.dirname(__file__), 'conf', 'storage_conf.ini')
        self._conf.read(conf_path)
        self._pool_folder = pool_folder or os.path.abspath(os.path.join(os.path.dirname(__file__), self._conf['MEMORY_MANAGER']['POOL_FOLDER']))
        self._block_file = block_file or os.path.abspath(os.path.join(os.path.dirname(__file__), self._conf['MEMORY_MANAGER']['BLOCK_FILE']))
        self._block_header_length = int(self._conf.get('MEMORY_MANAGER', 'BLOCK_HEADER_LENGTH', fallback=0)) or 10
    ...
    def allocate_block(self, block_size):
        ...
        output_bytearray.extend((('%0{}d'.format(self._block_header_length)) % len(string)).encode('utf-8'))
    ...
    def _allocate_new_pool(self):
        pool_path = os.path.abspath(os.path.join(self._pool_folder, 'pool_{}'.format(self._next_pool_id)))
    ...
    def _bootstrap(self):
        ...
        while index < length:
            ...
            data = bytes[index + self._block_header_length: index + self._block_header_length + data_length]
